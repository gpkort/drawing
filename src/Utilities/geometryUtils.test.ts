import {len, midpoint, rotate, LineStyle, getLineStyle} from './';
...
describe("rotate", () => {
    ...
    const mp = midpoint(line.begin, line.end)
    expect(mp[0]).toEqual(0)
    expect(mp[1]).toEqual(0)
    const newLine = rotate(line, Math.PI / 2, mp)
